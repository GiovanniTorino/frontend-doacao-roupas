...
@Component({
  selector: 'app-agasalhos',
  standalone: true,
  imports: [CommonModule, ProdutoCardComponent, DrawerComponent],
  template: `
    <div class="page">
      <app-drawer #drawer></app-drawer>
      <div class="topbar">
        <button class="menu-btn" (click)="drawer.abrirMenu()"><span></span><span></span><span></span></button>
        <img class="avatar" src="https://i.pravatar.cc/80?img=47" alt="Perfil" (click)="ir('/perfil')" />
      </div>
      <div class="cat-banner" style="background:#4527a0">
        <div class="banner-overlay" style="background:#4527a0"></div>
        <h1>Agasalhos</h1>
      ...
      <div *ngIf="!carregando && items.length > 0" class="items-grid">
        <app-produto-card *ngFor="let item of items" [peca]="item" categoria="agasalhos" emoji="🧥" (adquirido)="mostrarToast($event)" />
      </div>
      <div class="toast" *ngIf="toast">✅ {{ toast }}</div>
    </div>
  `
})
export class AgasalhosComponent implements OnInit {
  items: Peca[] = [];
  carregando = true;
  erro = '';
  toast = '';
  ...
  ngOnInit(): void {
    this.api.getAgasalhos().subscribe({
